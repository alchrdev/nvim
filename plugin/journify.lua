...
--- @param date_str string La fecha en formato YYYY-MM-DD
--- @return string
...
local function open_or_create_note_for_date(date)
    ...
    if not folder_exists then
        print("Error: La carpeta 'apsn' no existe en el directorio raíz")
        return
    end
    local file_name = folder_path .. "/" .. date .. ".md"
    file_name = file_name:gsub("/", vim.fn.has('win32') == 1 and "\\" or "/")
    ...
    if file then
        ...
        if new_file then
            ...
            print("Error creando el archivo: " .. file_name)
        end
    end
end
